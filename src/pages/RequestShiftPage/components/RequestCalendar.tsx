...
export default function ShiftCalendar() {
  ...
  const onDragEnd = useCallback(
    ({ source, destination, draggableId }: DropResult) => {
      ...
      const dragedNurse = requestShift.divisionShiftNurses[sourceDivision].find((x) => x.shiftNurse.shiftNurseId === parseInt(draggableId))!.shiftNurse;
  ...
  return requestShift && foldedLevels && wardShiftTypeMap && currentShiftTeam ? (
    <div id="calendar" className="flex">
      <div ref={clickAwayRef} className="flex flex-col">
        <div className="z-20 my-[.75rem] flex h-[1.875rem] items-center gap-5 bg-main-bg pr-4">
          <div className="flex gap-5">
            ...
            <div className="w-[1.875rem] text-center font-apple text-[1rem] font-medium text-sub-3">연동</div>
                                  ...
                                  >
                                    ...
                                    <div className="flex w-[1.875rem] shrink-0 items-center justify-center text-center font-apple text-[1.25rem] text-sub-1">
                                      {currentShiftTeam.nurses.find((x) => x.nurseId === row.shiftNurse.nurseId)?.isConnected ? (
                                        <LinkedIcon className="size-6" />
                                      ) : (
                                        <UnlinkedIcon className="size-6" />
                                      )}
                                    </div>
                                    <div className="flex h-full px-[1.0625rem]">
                                      {row.wardReqShiftList.map((current, date) => {
                                        const requestDutyRequest =
                                          dutyRequestList?.find((x) => x.nurseId === row.shiftNurse.nurseId && x.date - 1 === date) || null;
                                        const isSaturday = requestShift.days[date].dayType === 'saturday';
                                        const isSunday = requestShift.days[date].dayType === 'sunday' || requestShift.days[date].dayType === 'holiday';
                                        const isFocused = focus?.shiftNurseId === row.shiftNurse.shiftNurseId && focus.day === date;
                                        return (
                                          <div
                                            key={date}
                                          ...
                                          >
                                            <ShiftBadge
                                              id={rowIndex === 0 && date === 0 ? 'cell_sample' : undefined}
                                              onClick={() => {
                                                if (readonly) return;
                                                changeFocus?.({
                                              ...
                                              isOnlyRequest={current === null && requestDutyRequest !== null}
        ...
        <div className="max-h-[calc(100vh-9rem)] overflow-scroll rounded-[1.25rem] bg-white py-[.0625rem] scrollbar-hide">
          {dutyRequestList?.map((dutyRequest, i) => {
            const focus: Focus = {
              ...
              shiftNurseId: requestShift.divisionShiftNurses.flatMap((x) => x).find((x) => x.shiftNurse.nurseId === dutyRequest.nurseId)?.shiftNurse
                .shiftNurseId!,
              day: dutyRequest.date - 1,
            };

            return (
              <div key={i} className="flex h-[3.25rem] items-center border-b-[.0625rem] border-sub-4.5 pl-[1.875rem] pr-[.75rem] last:border-b-0">
                <p
                ...
                <div className="ml-auto flex h-7 w-[5.625rem] items-center justify-center gap-[.125rem] rounded-[.3125rem] border-[.0313rem] border-sub-4 bg-sub-5 p-[.125rem]">
                  <button
                    className={twMerge(
