...
export const TutorialOverlay = ({ config, closeCallback }: TutorialOverlayProps) => {
  ...
  return (
    <>
      <div
        id="TutorialOverlay"
        className="fixed top-0 left-0 z-998 h-screen w-screen bg-[#00000020]"
        ref={overlayElement}
      ></div>
      <div
        id="InfoBox"
        className="width-[20rem] font-apple absolute top-25 z-999 flex min-h-30 flex-col rounded-[.625rem] bg-white p-4 shadow-[5px_5px_15px_0px_rgba(149,81,146,0.3)]"
        ref={infoBoxElement}
      >
        <div className="flex flex-1 flex-col">
          <div id="InfoTitle" className="flex items-center">
            <p className="text-main-1 truncate text-[1.25rem] font-semibold">
              {config.steps.get(step)?.title}
            </p>
            <button
              className="text-main-2 ml-auto text-[.75rem] font-medium underline underline-offset-[.1rem]"
              onClick={skip}
            >
              건너뛰기
            </button>
          </div>
          <div id="InfoContent" className="scrollbar-hide mt-4 flex-1 overflow-y-scroll">
            {config.steps
              .get(step)
              ?.info?.split('\n')
              .map((x, i) => (
                <p key={i} className="text-sub-1 text-[1rem] font-medium">
                  {x}
                </p>
              ))}
          </div>
        ...
        <div id="BoxFooter" className="flex items-center justify-between">
          <div id="InfoSteps" className="text-sub-2.5 text-[.75rem] font-medium">
          ...
          <div id="ButtonWrapper" className="flex gap-[.625rem]">
            {config.steps.get(step)?.ctaUrl && config.steps.get(step)?.ctaText && (
              <a
                className="bg-main-1text-white border-main-1 bg-main-1 flex h-6 items-center justify-center rounded-[.3125rem] border-[.0625rem] px-[.375rem] text-[.875rem] font-medium text-white transition-all"
            ...
            <button
              className="border-main-1 text-main-1 hover:enabled:bg-main-1 disabled:border-sub-3 disabled:text-sub-3 flex h-6 items-center justify-center rounded-[.3125rem] border-[.0625rem] px-[.375rem] text-[.875rem] font-medium transition-all hover:enabled:text-white"
            ...
            <button
              className={twMerge(
                'border-main-1 text-main-1 hover:enabled:bg-main-1 disabled:border-sub-3 disabled:text-sub-3 flex h-6 items-center justify-center rounded-[.3125rem] border-[.0625rem] px-[.375rem] text-[.875rem] font-medium transition-all hover:enabled:text-white',
