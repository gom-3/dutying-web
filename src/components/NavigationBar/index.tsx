    ...
    <div className="group">
      {isFold && (
        <div className="fixed z-10 ml-[.9375rem] mt-[.625rem] flex items-center bg-white font-apple text-[1rem] font-medium">
          <MenuIcon className="cursor-pointer w-[1.875rem] h-[1.875rem]" />
        </div>
      )}
      <div
      ...
      >
        <div onClick={() => setIsFold(!isFold)}>
          <FoldIcon
            ...
            } w-[1.875rem] h-[1.875rem] absolute right-[.875rem] top-[.8125rem] cursor-pointer duration-300`}
          />
        </div>
        <div
          onClick={() => navigate('/')}
        ...
        >
          {pathname === HOME ? <FullLogo className="w-[5.6875rem] h-[1.5625rem]"/> : <FullLogoTransparent className="w-[5.6875rem] h-[1.5625rem]"/>}
          {pathname === HOME && (
